...
export default function DiagnosticDetail() {
  ...
  const calculateScores = (results: any) => {
    if (!results) return { globalScore: 0, categories: [] };

    // Implémentation simplifiée - à adapter selon votre structure réelle de données
    const categories = Object.entries(results).map(([category, score]: [string, any]) => ({
      name: category,
          ...
          <Card>
            ...
            <CardContent className="pt-6">
              {diagnostic.recommendations ? (
                <div className="space-y-4">
                  {Object.entries(diagnostic.recommendations).map(([key, value]: [string, any], index) => (
